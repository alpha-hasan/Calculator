...
export default function Calculator() {
    ...
    const ShowResultOnDisplay = (key) => {
        ...
        if (!operators.includes(key.target.name) && key.target.name !== 'C' && key.target.name !== '=') {
            ...
            // Limiting the amount of numbers the user can type

            if (result.length > 16) {
                alert("Limit Reached! You Can't add more than 17 numbers");
            }

            // limiting the amount of decimals the user can type

            else if (key.target.name == '.' && result !== '0' && result.charAt(result.length - 1) !== '.' && !operators.includes(result.charAt(result.length - 1))) {
        ...
        if (result !== '0' && operators.includes(key.target.name) && !operators.includes(result.charAt(result.length - 1)) && result.charAt(result.length - 1) !== '.') {
            setResult(result + key.target.name);
        }
        ...
        if (key.target.name === '=') {
            for (let i = 0; i < result.length; i++) {
                if (result !== '0' && operators.includes(result[i]) && !operators.includes(result.charAt(result.length - 1)) && result.charAt(result.length - 1) !== '.') {
                    ...
                    // calculate hone k bad ka behavior acha nhi.. Needs to be changed
                }
            }
        }
    }
